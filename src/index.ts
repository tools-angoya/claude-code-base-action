...
async function run() {
  try {
    ...
    if (promptConfig.isBoomerangTask && promptConfig.boomerangConfig) {
      console.log(`🪃 ブーメランタスクを実行中: ${promptConfig.boomerangConfig.targetMode}モード`);
      ...
      console.log(`📊 タスク分析: 複雑度=${promptConfig.taskAnalysis?.complexity.level}, 推奨モード=${promptConfig.taskAnalysis?.recommendedMode.mode}`);
