...
} from "../src/auto-orchestrator";
import { analyzeTaskSync } from "../src/task-analyzer";
  ...
  describe("orchestrateTask", () => {
    ...
    it("should orchestrate complex tasks into subtasks", async () => {
      const taskDescription =
        "新しいユーザー認証システムを実装してください。データベース設計、API実装、テストが必要です。";
    ...
    it("should analyze task complexity correctly", async () => {
      const complexTask =
        "フルスタックWebアプリケーションを開発してください。React、Node.js、PostgreSQL、Docker、CI/CDパイプラインを含む";
      const simpleTask = "README.mdファイルを更新してください";

      const complexAnalysis = analyzeTaskSync(complexTask);
      const simpleAnalysis = analyzeTaskSync(simpleTask);
    ...
    it("should recommend appropriate modes for different tasks", async () => {
      ...
      const designAnalysis = analyzeTaskSync(designTask);
      const codeAnalysis = analyzeTaskSync(codeTask);
      const debugAnalysis = analyzeTaskSync(debugTask);
      const questionAnalysis = analyzeTaskSync(questionTask);
    ...
    it("should handle task dependencies correctly", async () => {
      ...
      const analysis = analyzeTaskSync(taskDescription);

      if (analysis.requiresOrchestration) {
        const implementationTask = analysis.subTasks.find(
          (t) => t.id === "implementation",
        );
        ...
        if (implementationTask && testingTask) {
          ...
          expect(testingTask.priority).toBeGreaterThan(
            implementationTask.priority,
          );
        }
      }
    });
